...
import 'package:trip/screens/photo_edit_screen.dart';
...
class _PhotoViewScreenState extends State<PhotoViewScreen>
  ...
  Future<void> _updatePhotoLocation(gm.LatLng location, [String? address]) async {
    setState(() {
      _photoLocation = location;
    });
    if (currentPhoto is Map<String, dynamic>) {
      await _photoService.updatePhotoLocation(location, currentPhoto, address: address);
      _showCustomMessage('Геолокация и адрес обновлены на сервере');
    } else if (currentPhoto is AssetEntity) {
      _onPressUpload(address: address);
      _showCustomMessage('Геолокация установлена');
    }
  }

  void _openPhotoEditor() async {
    final photoForEditing = currentPhoto;
    if (photoForEditing == null) return;
    final editedFile = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) {
          if (photoForEditing is AssetEntity) {
            return PhotoEditScreen(localPhoto: photoForEditing);
          } else if (photoForEditing is Map<String, dynamic>) {
        ...
        return StatefulBuilder(
          builder: (BuildContext context, StateSetter setStateModal) {
            Future<void> _onSearchChanged(String input) async {
              ...
              final url =
                  "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$input&language=ru&components=country:ru&key=$googleApiKey";
              try {
            ...
            Future<void> _onPredictionTap(Map<String, dynamic> prediction) async {
              ...
              final detailsUrl =
                  "https://maps.googleapis.com/maps/api/place/details/json?place_id=$placeId&language=ru&key=$googleApiKey";
              try {
                            ...
                            return Column(
                              ...
                              children: [
                                const Padding(
                                  ...
                                  child: Text(
                                    "Выберите ранее использованный адрес:",
                      ...
                      Padding(
                        ...
                        child: ElevatedButton.icon(
                          ...
                          onPressed: () {
                            Navigator.pop(context);
                            if (tempLocation != null) {
                              _updatePhotoLocation(tempLocation!, searchController.text);
                            } else {
                              _showCustomMessage("Не удалось получить координаты для выбранного адреса.",
...
String _permissionType(Map<String, dynamic> sharedWith, String userId) {
  ...
  void _onPressUpload({String? address}) async {
    if (currentPhoto is AssetEntity) {
      setState(() {
        _isLoading = true;
      });
      try {
        await _photoService.saveLocalPhotoToServer(currentPhoto, _photoLocation, address: address);
        setState(() {
          _isLoading = false;
        });
