...
class PhotoService {
  ...
  Future<void> saveLocalPhotoToServer(AssetEntity localPhoto, gm.LatLng? photoLocation, {String? address}) async {
    try {
      final existingPhoto = await FirebaseFirestore.instance
          .collection('photos')
          .where('id', isEqualTo: localPhoto.title)
      ...
      if (existingPhoto.docs.isEmpty) {
        await pickAndUploadImage(localPhoto, photoLocation, address: address);
  ...
  Future<void> updatePhotoLocation(gm.LatLng location, Map<String, dynamic>? serverPhoto, {String? address}) async {
  ...
  Future<List<String>> fetchUsedAddressesForCurrentUser() async {
    final userUid = FirebaseAuth.instance.currentUser?.uid;
    if (userUid == null) return [];
    final snapshot = await FirebaseFirestore.instance
        .collection('photos')
        .where('address', isGreaterThan: '')
        .get();
    final addresses = snapshot.docs
        ...
        .map((photo) => photo['address'] as String)
        .toSet()
        .toList();
    return addresses;
  }
  ...
  try {
    ...
    var stream = editedFile.openRead().transform(
      StreamTransformer<List<int>, Uint8List>.fromHandlers(
        handleData: (data, sink) {
          sink.add(Uint8List.fromList(data));
        },
    ...
    print("Ошибка при загрузке отредактированного файла: $e");
    return '';
  }
}
  ...
  Future<void> updatePhotosSharingForFolder(String folderId, Map<String, dynamic> newShare) async {
    try {
      final folderSnapshot = await FirebaseFirestore.instance.collection('folders').doc(folderId).get();
      String? ownerUid;
      if (folderSnapshot.exists) {
        final folderData = folderSnapshot.data();
        ownerUid = folderData?['owner'];
      }
      ...
      for (var doc in snapshot.docs) {
        ...
        if (ownerUid != null && !unionShared.containsKey(ownerUid)) {
          unionShared[ownerUid] = {
            'addPhotos': true,
            'deleteFolder': true,
            'deletePhotos': true,
